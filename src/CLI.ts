...
// #region fs/path
function promisify<ARGS extends any[], RETURN> (fn: (...args: [...ARGS, (err: Error | null, result: RETURN) => any]) => void, ...args: ARGS): Promise<RETURN> {
	return new Promise<RETURN>((resolve, reject) => fn.call(null, ...args, (err: Error | null, result: RETURN) => err ? reject(err) : resolve(result)));
}

namespace File {
	export async function stat (file: string) {
		return promisify(fs.stat, file).catch(() => null);
	}

	export async function children (dir: string) {
		return promisify<[fs.PathLike], string[]>(fs.readdir, dir)
...
function color (text: string, color: keyof typeof import("ansicolor")): string {
	if (!chalk && !ansicolor)
		return text;
